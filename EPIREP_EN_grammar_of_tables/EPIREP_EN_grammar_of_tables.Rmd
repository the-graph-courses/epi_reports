...
1. **Use the `gt()` function to create basic tables**: Learn to initiate table creation in R using `gt()` for data display.

2. **Group columns under spanner headings**: Master grouping related columns under shared headings for enhanced table organization.

3. **Add stubs**: Implement stubs to provide row labels or descriptions in tables for better clarity.

4. **Relabel column names**: Learn to rename column labels for improved readability and context in tables.

5. **Add summary rows for groups**: Understand adding aggregate data rows like sums or averages per group for comprehensive analysis.
...
Our data comes from the **Malawi HIV Program** and covers antenatal care and HIV treatment during 2019. We will focus on quarterly regional and facility-level aggregates (available [here](https://dms.hiv.health.gov.mw/dataset/?tags=HIV&res_format=XLSX&year=2019)).
