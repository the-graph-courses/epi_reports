...
```{r}
ggplot(aus_long) + 
  geom_line(aes(x = year, y = cases, colour = region, group = region))
...
ggplot(aus_long, aes(x = year, y = cases, colour = region, group = region)) +
  geom_line() +
  geom_point() +
  geom_text(aes(label = cases), size = 3.3, nudge_y = 20)
```
...
ggplot(aus_long, aes(x = year, y = cases, colour = region, group = region)) +
  geom_line() +
  geom_point() +
  geom_text(data = even_years, aes(label = cases), size = 2.8, nudge_y = 20)
...
ggplot(aus_long, aes(x = year, y = cases, colour = region, group = region)) +
  ...
  geom_point() + 
  geom_text(data = even_years, aes(label = cases), size = 2.8, nudge_y = 20)  + 
  geom_text(data = odd_years, aes(label = cases), size = 2.8, nudge_y = -20)
```
...
```{r}
hiv_data_brazil %>% 
  separate(col = new_hiv_cases, into = c("cases", "cases_lower", "cases_upper"), 
           sep = "\\[|–")
```
...
hiv_data_brazil_clean <- 
  ...
  mutate(across(.cols = c(cases, cases_lower, cases_upper), 
                .fns = ~ as.numeric(str_remove_all(.x, "[^0-9]"))
                ))
```
...
hiv_data_brazil_clean %>% 
  ...
  ggplot(aes(y = cases, x = year, color = sex, fill = sex)) + 
  geom_line() + 
  geom_ribbon(aes(ymin = cases_lower, ymax = cases_upper), alpha = 0.4)
...
```{r}
mal %>% 
  count(date_positive_test, name = "cases") %>% kable()
...
mal_notif_count <- 
  ...
  complete(date_positive_test = seq.Date(from = min(mal$date_positive_test), 
                                         to = max(mal$date_positive_test), 
                                         by = "day"), 
           fill = list(cases = 0))
...
ggplot(mal_notif_count) + 
  geom_line(aes(x = date_positive_test, y = cases))
  
  # line plot of cases against date_positive_test
```
...
```{r}
mal_notif_count %>% 
  mutate(month = floor_date(date_positive_test, "month"))
```
...
```{r}
mal_notif_count <- mal_notif_count %>%
  mutate(roll_cases = rollmean(cases, k = 7, fill = NA, align = "right"))


mal_notif_count %>% kable()
# Use head(mal_notif_count_right, 15) to see the first 15 days
```
...
mal_notif_count_cumul %>% kable()
```

‣ **Identifying the Need for a Secondary Y-Axis** - Plot both daily cases and cumulative cases on the same y-axis.

```{r}
ggplot(mal_notif_count_cumul) + 
  geom_line(aes(x = date_positive_test, y = cases)) + 
  geom_line(aes(x = date_positive_test, y = cumul_cases))

# Plot total malaria cases
...
ggplot(mal_notif_count_cumul, aes(x = date_positive_test)) +
  ...
  geom_line(aes(y = cumul_cases/ 49.97), color = "red") # divide cumul_cases by scale factor
```
...
ggplot(mal_notif_count_cumul, aes(x = date_positive_test)) +
  ...
  scale_y_continuous(sec.axis = sec_axis(trans = ~ .x * 49.97, name = "Cumulative Cases"))
...
-   Compare time series with distinct scales using dual axes and sec_axis() ![](images/blank.png)
