...
1.  **Understand Barplots, Col Plots, and Histograms**
    -   Gain the ability to distinguish between these plot types and understand the specific contexts in which each should be used for data visualization.
2.  **Dive Deeper into Grouped Plots**
    -   Learn how to create and interpret grouped plots, which allow for the comparison of different categories or groups within a dataset.
3.  **Enhance Barplots with Error Bars**
    -   Develop the skills to add error bars to barplots to represent the variability or uncertainty within the data, providing a clearer picture of the reliability of the data.
4.  **Explore New Forms with Area Plots**
    -   Explore the construction and usage of area plots, and how they can be utilized to display quantitative data graphically over time or other variables.
5.  **Venture into Circular Plots: Pie Charts and Donut Plots**
    -   Understand the design and application of pie charts and donut plots for showing proportional relationships within a dataset in a circular format.
...
However, if your data is already summarized or includes specific values for the y-axis (the height of the bars), you should use `geom_col()`. This function is designed to plot pre-calculated values, great for when you have a clear y variable to display against a categorical x variable. First let's calculate the total number of cases in each period:
...
```{r}
# with geom text and adjustments (fixed aesthetics)
period_sum %>% 
  # Pass the summarised data to ggplot for visualisation
  ggplot(
    # Within aes(), specify that the x-axis will represent the 'period', 
...
## Diving Deeper into grouped plots
...
In summary, `geom_col()` provides a suite of options for visualizing categorical data in different contexts. The choice between stacked, normalized stacked, and grouped plots depends on your data and the insights you want to highlight. The choice between `geom_bar()` and `geom_col()`, and between stacked, normalized stacked, and grouped plots, depends on the specifics of your data and what you want to emphasize in your visualization.
...
Stacked plots offer a holistic view of category totals while visually segregating contributions from different sub-categories. With `geom_col()`, simply map a variable to the `fill` aesthetic to create divisions in the bars representing your sub-categories. Let's take advantage of this and explore share of total cases for each facility in each period. First let's summarize our data by `orgunit` and by `period`, this is extremely important to properly visualize the differences:

```{r}
orgunit_period_sum <- tb %>% 
...
THIS DOESN'T WORK. FOR STACKED BARS, NEED TO CALCULATE CUMULATIVE SUM FOR EACH CATEGORY.
...
First we need to summarize our data:
...
tb %>% 
  ...
  geom_text(data = tb_label, 
            aes(x = orgunit, 
                y = maxval, 
                label=maxval), vjust=1.6, color="white",
            position = position_dodge(0.9), size=3.5)
```
