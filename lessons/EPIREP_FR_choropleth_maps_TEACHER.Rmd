...
‣ Maîtriser les fonctions `ggplot()` et `geom_sf()` pour la visualisation de cartes.

‣ Fusionner les frontières et les données liées aux maladies en fonction des niveaux administratifs.

‣ Implémenter des échelles de couleurs pour les types de données continus et discrets.

‣ Créer des petites multiples de cartes.
...
ggplot() +  # Nous initialisons une visualisation avec ggplot
  geom_sf(data = nga_adm1) +  # Nous ajoutons notre shapefile à la visualisation avec geom_sf()
...
ggplot(data = malaria2) + 
  geom_sf(aes(fill = cases_2021)) +   # Remplissage basé sur les cas de 2021
...
ggplot(data=malaria2) + 
  ...
  ggspatial::annotation_scale(lcoation = "tr")
...
ggplot(data = malaria2) +
  geom_sf(aes(fill = cases_2021))+
  scale_fill_continuous(low = "yellow", high = "red", name = "Cas")
...
ggplot(data = malaria3) +
  ...
  scale_fill_manual(values = custom_palette, name = "Niveaux de risque") +
...
malaria3_longer %>%
  mutate(risk = case_when(
    cases >= median(cases) ~ 'haut',
    cases < median(cases) ~ 'bas'
  )) -> malaria3_longer2
```
